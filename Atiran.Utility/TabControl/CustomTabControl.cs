    ...
    public class CustomTabControl : TabControl
    {
        ...
        protected override void OnMouseClick(MouseEventArgs e)
        {
                     ...
                     GetTabCloserRect(index).Contains(MousePosition))
            {
                ...
                SelectedIndex = i >= index ? i > 0 ? i - 1 : i : i;
            }
            else
            {
                //	Fire the base event
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (_StyleProvider.ShowTabCloser)
            {
                ...
                if (tabRect.Contains(MousePosition)) Invalidate();
            }
            ...
            if (Width > 0 && Height > 0)
            {
                ...
                if (TabCount > 0)
                {
                    ...
                    if (Multiline)
                        ...
                        for (var index1 = TabCount - 1; index1 >= 0; index1--)
                            if (index1 != SelectedIndex)
                                DrawTabPage(index1, _TabBufferGraphics);
        ...
        private Rectangle GetTabImageRect(GraphicsPath tabBorderPath)
        {
            ...
            if (Alignment <= TabAlignment.Bottom)
            {
                ...
                else
                {
                    ...
                    if (_StyleProvider.ShowTabCloser && !RightToLeftLayout) imageRect.X -= 10;
                }
            }
            else
            {
                ...
                else
                {
                    ...
                    //	Move it in further to allow for the tab closer
                    if (_StyleProvider.ShowTabCloser && !RightToLeftLayout) imageRect.Y -= 10;
                }
            }
        ...
        public Rectangle GetTabCloserRect(int index)
        {
            ...
            using (var path = _StyleProvider.GetTabBorder(index))
            {
                ...
                if (Alignment <= TabAlignment.Bottom)
                {
                    ...
                    //else
                    //{
                    closerRect = new Rectangle((int) rect.Right + 3,
                        (int) rect.Y + (int) Math.Floor((double) ((int) rect.Height - 6) / 2) - 3, 11, 11);
                    while (!path.IsVisible(closerRect.Right, closerRect.Y) && closerRect.Right > -6) closerRect.X -= 1;
                    closerRect.X -= 10;
                    //}
                }
                else
