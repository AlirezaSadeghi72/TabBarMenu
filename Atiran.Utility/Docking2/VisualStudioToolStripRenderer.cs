...
namespace Atiran.Utility.Docking2
{
    public class VisualStudioToolStripRenderer : ToolStripProfessionalRenderer
    {
            ...
            : base(new VisualStudioColorTable(palette))
        {
            ...
            CloseButtom = ImageServiceHelper.GetImage(Resources.MaskTabClose, _palette.TabSelectedInactive.Button,
        ...
        protected override void OnRenderMenuItemBackground(ToolStripItemRenderEventArgs e)
        {
            ...
            if (e.Item.Enabled)
            {
                ...
                else if (e.Item.BackColor != SystemColors.Control)
                {
                    // Border pen and fill brush.
                    var pen = ColorTable.MenuItemBorder;
                    var brushBegin = e.Item.BackColor;
                    var brushEnd = Color.Empty;
        ...
        protected override void OnRenderStatusStripSizingGrip(ToolStripRenderEventArgs e)
        {
            ...
            var g = e.Graphics;
            var statusStrip = e.ToolStrip as StatusStrip;
            ...
            if (statusStrip != null)
            {
                var sizeGripBounds = statusStrip.SizeGripBounds;
                if (!LayoutUtils.IsZeroWidthOrHeight(sizeGripBounds))
                {
                    var whiteRectangles = new Rectangle[baseSizeGripRectangles.Length];
                    var greyRectangles = new Rectangle[baseSizeGripRectangles.Length];

                    for (var i = 0; i < baseSizeGripRectangles.Length; i++)
                    {
                        var baseRect = baseSizeGripRectangles[i];
        ...
        protected override void OnRenderGrip(ToolStripGripRenderEventArgs e)
        {
            ...
            if (numRectangles > 0)
            {
                ...
                Brush b = _gripBrush;
                for (var i = 0; i < numRectangles - 1; i++) g.FillRectangle(b, shadowRects[i]);

                for (var i = 0; i < numRectangles; i++) shadowRects[i].Offset(xOffset, -2);

                g.FillRectangles(b, shadowRects);

                for (var i = 0; i < numRectangles; i++) shadowRects[i].Offset(-2 * xOffset, 0);
        ...
        protected override void OnRenderItemCheck(ToolStripItemImageRenderEventArgs e)
        {
            ...
            using (var imageAttr = new ImageAttributes())
            {
                ...
                var borderColor = _palette.CommandBarMenuPopupDefault.Border;
                ...
                e.Graphics.DrawImage(e.Image, e.ImageRectangle, 0, 0, e.Image.Width, e.Image.Height, GraphicsUnit.Pixel,
                    imageAttr);
            ...
            Color brushMiddle, Color brushEnd, Color penColor, bool glass)
        {
            ...
            if (brushMiddle.IsEmpty && brushEnd.IsEmpty) graphics.FillRectangle(new SolidBrush(brushBegin), rect);
            ...
            if (penColor.A > 0) graphics.DrawRectangle(new Pen(penColor), rect);
        }
