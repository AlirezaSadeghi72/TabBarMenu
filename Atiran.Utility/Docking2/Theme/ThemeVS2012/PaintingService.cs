...
namespace Atiran.Utility.Docking2.Theme.ThemeVS2012
{
    public class PaintingService : IPaintingService
    {
        ...
        public SolidBrush GetBrush(Color color)
        {
            ...
            if (_brushCache.ContainsKey(key)) return _brushCache[key];
        ...
        public Pen GetPen(Color color, int thickness)
        {
            ...
            if (_penCache.ContainsKey(key)) return _penCache[key];
        ...
        public void CleanUp()
        {
            foreach (var pen in _penCache) pen.Value.Dispose();

            _penCache.Clear();

            foreach (var brush in _brushCache) brush.Value.Dispose();
